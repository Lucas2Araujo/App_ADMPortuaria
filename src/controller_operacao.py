...
async def registrar_desatracacao(session, imo_id: str) -> Optional[OperacaoLogDTO]:
    ...
    """
    stmt = select(Atracacao).filter(Atracacao.navio_imo_id == imo_id, Atracacao.data_hora_fim.is_(None))
    ...
    result_vaga = await session.execute(select(Vaga).filter(Vaga.id == atracacao.vaga_id))
...
async def obter_painel_vagas_dto(session) -> list[VagaDTO]:
    ...
    result_atracacoes = await session.execute(select(Atracacao).filter(Atracacao.data_hora_fim.is_(None)))
    ...
    if imos_ativos:
        stmt = select(Navio).options(joinedload(Navio.cargas)).filter(Navio.imo_id.in_(imos_ativos))
...
async def obter_log_operacoes_dto(session) -> list[OperacaoLogDTO]:
    ...
    """
    stmt = select(Atracacao, Navio.nome).outerjoin(Navio, Atracacao.navio_imo_id == Navio.imo_id)
    result = await session.execute(stmt)
...
async def liberar_vaga_individual(session, vaga_id: int):
    ...
    if vaga and vaga.status == StatusVaga.OCUPADA:
        ...
        if atracacao:
            atracacao.data_hora_fim = datetime.now()
            result_navio = await session.execute(select(Navio).filter(Navio.imo_id == atracacao.navio_imo_id))
            navio = result_navio.scalars().first()
            if navio:
                navio.status = StatusNavio.FINALIZADO
        vaga.status = StatusVaga.LIVRE
...
async def obter_contadores_dashboard(session) -> dict:
    ...
    vagas_ocupadas_res = await session.execute(select(func.count(Vaga.id)).filter(Vaga.status == StatusVaga.OCUPADA))
    vagas_ocupadas = vagas_ocupadas_res.scalar()

    vagas_livres = total_vagas - vagas_ocupadas

    total_validado_res = await session.execute(select(func.count(Navio.imo_id)).filter(Navio.status == StatusNavio.VALIDADO))
    total_validado = total_validado_res.scalar()

    total_pendente_res = await session.execute(select(func.count(Navio.imo_id)).filter(Navio.status == StatusNavio.PENDENTE))
    total_pendente = total_pendente_res.scalar()

    total_finalizado_res = await session.execute(select(func.count(Navio.imo_id)).filter(Navio.status == StatusNavio.FINALIZADO))
